...
public class ScapegoatTree<Key extends Comparable<Key>, Value> {
    ...
    private Node put(Node x, Key key, Value val) {
        ...
        x.height = (Math.max(height(x.left), height(x.right)))+1; //check height of children if null height=1
        x.size = 1 + size(x.left) + size(x.right); //check size of children and add 1

        if (x.height - 1 > alpha * log2(x.size)){ //check if the tree follows size invariant
            x = rebuild(x);
        }
        return x;
    }
    ...
    private Node balanceNodes(ArrayList<Node> nodes, int lo, int hi) {
        ...
        Node right = balanceNodes(nodes,mid+1,hi);
         Node nMid = new Node(nodes.get(mid).key, nodes.get(mid).val);
         nMid.left = left;
         nMid.right = right;
         nMid.size = size(left) + size(right) + 1;
         nMid.height = (Math.max(height(nMid.left), height(nMid.right)))+1;
